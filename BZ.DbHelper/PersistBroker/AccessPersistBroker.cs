...
{
    public class AccessPersistBroker : AbstractPersistBroker, IPersistBroker
    {
        public AccessPersistBroker(string connectionString)
            : base(new OleDbConnection(connectionString))
        {

        }

        public AccessPersistBroker(string connectionString, System.Globalization.CultureInfo cultureInfo)
        ...
        /// <returns></returns>
        private OleDbType CTypeToSType(Type cType)
        {
        ...
        /// <returns></returns>
        private void CommandParam(OleDbCommand command, string commandText, string[] parameterNames, Type[] parameterTypes, int[] parameterSizes, object[] parameterValues)
        {
            ...
            for (int i = 0; i < parameterNames.Length; i++)
            {
                ...
                if (parameterSizes[i] > 0)
                    param = new OleDbParameter(parameterNames[i], CTypeToSType(parameterTypes[i]), parameterSizes[i]);
                else
                    param = new OleDbParameter(parameterNames[i], CTypeToSType(parameterTypes[i]));
        ...
        /// <param name="parameterValues"></param>
        private void CommandParam(OleDbCommand command, ArrayList parameters, ref object[] parameterValues)
        {
            for (int i = 0; i < parameters.Count; i++)
            {
                ...
                OleDbParameter param;
                if (proc_parm.Size > 0)
                    param = new OleDbParameter(proc_parm.Name, CTypeToSType(proc_parm.Type), proc_parm.Size);
                else
                    param = new OleDbParameter(proc_parm.Name, CTypeToSType(proc_parm.Type));
